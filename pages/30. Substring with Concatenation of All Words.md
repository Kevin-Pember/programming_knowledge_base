		...
		- Misalignment
			...
			- Examples
				...
				- Example 2 (Over word_length)
					...
					- words = ``["bar",  "bar", "foo"]``
					- The goal here is to show that we only need an offset between 0 to word_length - 1. Since ``"see"`` can be considered a word in and of itself we only need an offset of 2 to align our window. This stays the same with any offset because alignment needs to only
- Implementation
